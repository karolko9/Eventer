...
pub fn add_event_to_user(caller: Principal, event_id: u128) -> Result<(), ErrorUser> {
    ...
    USER_DATA_MODEL.with(|users| {
        ...
        if let Some(user) = users_map.get_mut(&caller) {
            ...
            Err(ErrorUser::UserNotFound)
        }
    })
}

pub fn add_hosting_event_to_user(caller: Principal, event_id: u128) -> Result<(), ErrorUser> {
...
pub fn add_event_online_to_user(caller: Principal, event_id: u128) -> Result<(), ErrorUser> {
...
pub fn add_hosting_event_online_to_user(caller: Principal, event_id: u128) -> Result<(), ErrorUser> {
...
pub fn update_user_database(user: Principal, user_dto: user_dto_request::UserDTO) -> Result<(), ErrorUser> {
        ...
        USER_DATA_MODEL.with(|user_data_model| {
            ...
            if let Some(existing_user_data) = user_data_map.get_mut(&user) {
                ...
                if let Err(_) = existing_user_data.set_location(user_dto.location.0, user_dto.location.1) {
                    return Err(ErrorUser::FailedToUpdateUserData);
                }
                ...
                let user_data = match UserDataModel::new(user_dto) {
                    Ok(data) => data,
                    Err(_) => return Err(ErrorUser::FailedToUpdateUserData),
                };
...
//SEEN
pub fn mark_event_as_seen(user: Principal, event_id: u128) -> Result<(), ErrorUser> {
    USER_DATA_MODEL.with(|user_data_model| {
        let mut user_data_map = user_data_model.borrow_mut();
