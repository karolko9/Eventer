...
#[ic_cdk::update]
pub fn add_tag_to_event(event_id: u128, tag: String) -> Result<(), String> {
    if event_repository::is_user_admin(ic_cdk::caller(), event_id) {
...
pub async fn verify_ticket_signature_by_admin(
    ...
    if event_repository::is_user_admin(ic_cdk::caller(), ticket.event_id.clone()) {
